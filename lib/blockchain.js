...
const getMNSubsidy = (nHeight = 0, nMasternodeCount = 0, nMoneySupply = 0) => {
  ...
  let ret = 0.0;
	...
	if(nHeight <= 75000 && nHeight >= params.RAMP_TO_BLOCK) {
        nSubsidy = 10 * COIN;
    ...
    let mNodeCoins = nMasternodeCount * 10000;
    if (mNodeCoins === 0) {
      ret = 0;
    } else {
...
const getSubsidy = (nHeight = 1) => {
  ...
  if (nHeight === 1) {
    ...
    nSlowSubsidy /= params.RAMP_TO_BLOCK;
    nSlowSubsidy *= nHeight;
  } else if (nHeight <= 75000 && nHeight >= params.RAMP_TO_BLOCK) {
    nSubsidy = 10;
